...
| Problema | Causa | Soluzione |
|---|---|---|
| ⚠️ Trovato 13/09/2026: `DATABASE_URL="file:./prisma/dev.db"` in `.env.example`/`.env` locale crea un `prisma/prisma/dev.db` annidato | Sia Prisma CLI che `resolveSqliteFilePath()` (vedi `src/lib/paths.ts`) risolvono un path relativo **rispetto alla cartella `prisma/`**, non alla root del progetto — stesso identico bug già corretto in `apps/license-server` (vedi `docs/license-server.md`), qui non ancora applicato | **Fix da applicare** (in sospeso, richiede conferma per toccare il DB reale locale): cambiare il valore in `"file:./dev.db"` in entrambi i file, e spostare `prisma/prisma/dev.db` (quello vero, coi dati) in `prisma/dev.db` |
...
| Build cross-arch di `pkg` fallisce ("spawn UNKNOWN"/"spawn ENOEXEC") | pkg per un'architettura CPU diversa da quella host deve "fabbricare" il binario eseguendolo — impossibile senza emulazione (QEMU/Rosetta) | Builda su un host/runner nativo di quella arch (vedi i workflow CI: `ubuntu-24.04-arm` per Linux arm64, `macos-latest`+Rosetta per macOS) |
